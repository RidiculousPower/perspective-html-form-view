
require_relative '../../../../lib/perspective/html/form.rb'

describe ::Perspective::HTML::Form::DataValidationObject do

  #####################################
  #  self.create_data_validation_class  #
  #  self.view_class                  #
  #  self.define_input                #
  #  self.define_subview             #
  #  initialize                       #
  #####################################
  
  it 'can create a validated data object class for a form or view class' do
    class ::Perspective::HTML::Form::DataValidationObject::MockView2
      include ::Perspective::HTML::View
    end
    class ::Perspective::HTML::Form::DataValidationObject::MockView
      include ::Perspective::HTML::View
      attr_view :some_subview, ::Perspective::HTML::Form::DataValidationObject::MockView2
    end
    new_instance = ::Perspective::HTML::Form::DataValidationObject.create_data_validation_class( ::Perspective::HTML::Form::DataValidationObject::MockView )
    new_instance.view_class.should == ::Perspective::HTML::Form::DataValidationObject::MockView
    new_instance.define_input( :some_input )
    new_instance.respond_to?( :some_input ).should == false
    new_instance.method_defined?( :some_input ).should == true
    new_instance.method_defined?( :some_input= ).should == true
    new_instance.define_subview( :some_subview )
    new_instance.method_defined?( :some_subview ).should == true
    new_instance.method_defined?( :some_subview= ).should == false
    data_object_instance = new_instance.new
    data_object_instance.some_input.should == nil
    data_object_instance.some_subview.should_not == nil
  end

end
